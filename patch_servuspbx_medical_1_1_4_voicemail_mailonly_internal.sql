...
UPDATE `ps_aors`
SET `mailboxes`=REPLACE(`mailboxes`, '@default', '@internal')
WHERE `mailboxes` LIKE '%@default%';
...
UPDATE `ps_aors` a
JOIN `ps_endpoints` p ON p.id=a.id
SET a.mailboxes = CONCAT(p.extension, '@internal')
WHERE p.active=1
  AND p.extension IS NOT NULL
  AND p.extension <> '';
